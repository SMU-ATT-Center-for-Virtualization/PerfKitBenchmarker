...
BENCHMARK_NAME = 'netperf_scheduler'
BENCHMARK_CONFIG = """
netperf_scheduler:
...
def Prepare(benchmark_spec):
    if len(benchmark_spec.vm_groups) <= 2:
        raise ValueError(
            'Scheduled Netperf benchmark requires more than two VM groups, '
            f'found {len(benchmark_spec.vm_groups)}'
        )

    background_tasks.RunThreaded(PrepareNetperf, benchmark_spec.vms)
    background_tasks.RunThreaded(PrepareClientVM, benchmark_spec.vms)
    background_tasks.RunThreaded(PrepareServerVM, benchmark_spec.vms)
...
def PrepareServerVM(server_vm):
    """Install netperf and start netserver processes on server_vm.

    (Does not perform manual firewall setup for contianer-optimized OS
    as in the original netperf_benchmark.py)
    """
    ...
    if vm_util.ShouldRunOnExternalIpAddress():
        server_vm.AllowPort(PORT_START, port_end)
...
def _HistogramStatsCalculator(histogram):
  ...
  latency_percentiles = FLAGS.netperf_histogram_percentiles
...
def RunNetperf(vm, benchmark_name, server_ips, num_streams, client_ips):
    ...
    enable_latency_histograms = enable_latency_histograms and (benchmark_name not in ['TCP_STREAM', 'UDP_STREAM'])

    confidence = (f"-I 99,5 -i {FLAGS.netperf_max_iter},3" if FLAGS.netperf_max_iter else "")
    verbosity = "-v2 " if enable_latency_histograms else ""

    remote_cmd_timeout = (FLAGS.netperf_test_length * (FLAGS.netperf_max_iter or 1) + 300)
    metadata = {
    ...
    for server_ip_idx, server_ip in enumerate(server_ips):
        ...
        if direction == "STREAM":
            if protocol == "UDP":
                ...
                send_size = FLAGS.netperf_sctp_stream_send_size_in_bytes
            else:
                raise ValueError(f"Unsupported protocol for netperf stream: {protocol}")
        ...
        if (FLAGS.netperf_numactl_membind or FLAGS.netperf_numactl_physcpubind or FLAGS.netperf_numactl_cpunodebind):
            numactl_prefix = "numactl "
            if FLAGS.netperf_numactl_physcpubind:
                numactl_prefix += f"--physcpubind {FLAGS.netperf_numactl_physcpubind} "
                metadata["netperf_physcpubind"] = FLAGS.netperf_numactl_physcpubind
            if FLAGS.netperf_numactl_cpunodebind:
                numactl_prefix += f"--cpunodebind {FLAGS.netperf_numactl_cpunodebind} "
                metadata["netperf_cpunodebind"] = FLAGS.netperf_numactl_cpunodebind
            if FLAGS.netperf_numactl_membind:
                numactl_prefix += f"--membind {FLAGS.netperf_numactl_membind} "
                metadata["netperf_membind"] = FLAGS.netperf_numactl_membind
    ...
    json_outs = [
        json.loads(remote_stdout_stderr[0]) for remote_stdout_stderr in remote_stdout_stderr_threads
    ]
    ...
    if len(parsed_output) == 1:
        ...
        if benchmark_name.upper() == "TCP_STREAM":
            output_samples.append(sample.Sample(
        ...
        throughput_samples, _, latency_histograms = (list(t) for t in zip(*parsed_output))
        ...
        if benchmark_name.upper() == 'TCP_STREAM':
            ...
            for throughput_sample in throughput_samples:
                ...
                if netperf_mss is None:
                    ...
                    raise ValueError("Netperf MSS values do not match for multiple netperf threads")
...
def Run(benchmark_spec):
    ...
    logging.info(f"Netperf Run Start - busy_vms: {busy_vms}")
...
def _PrepareRun(s_r, s_r_pairs, busy_vms):
    ...
    # get index of first available VMs in each subgroup
...
def RunClientServerVMs(client_vm, server_vm, results_q, vm_free_q, vm_pair):
...
def Cleanup(benchmark_spec):
    background_tasks.RunThreaded(CleanupServerVM, benchmark_spec.vms)
    background_tasks.RunThreaded(CleanupClientVM, benchmark_spec.vms)

def CleanupClientVM(vm):
    vm.RemoteCommand(f"sudo rm -rf {REMOTE_SCRIPT}")

def CleanupServerVM(vm):
    vm.RemoteCommand(f"sudo killall netserver")
