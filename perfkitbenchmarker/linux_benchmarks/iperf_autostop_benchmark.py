...
def _RunIperf(sending_vm, receiving_vm, receiving_ip_address, thread_count,
  ...
  try:
    stdout, _ = sending_vm.RemoteCommand("ip route show | grep default")
    default_interface = stdout.split()[4]
    stdout, _ = sending_vm.RemoteCommand(f'cat /sys/class/net/{default_interface.strip()}/mtu')
    print(f'MTU: {stdout}')
    metadata['sending_machine_mtu'] = int(stdout.strip())

    stdout, _ = receiving_vm.RemoteCommand("ip route show | grep default")
    default_interface = stdout.split()[4]
    stdout, _ = receiving_vm.RemoteCommand(f'cat /sys/class/net/{default_interface.strip()}/mtu')
    print(f'MTU: {stdout}')
    metadata['receiving_machine_mtu'] = int(stdout.strip())
  except:
    logging.warning("couldn't get mtu config")
  ...
  if protocol == TCP:
    ...
    if FLAGS.iperf_interval:
      if thread_count > 1:
        ...
        r = re.compile((
            ...
            r'(?P<err>\d+)\s+(?P<retry>\d+)\s+(?P<cwnd>-?\d+)(?P<cwnd_unit>\w*)\/(?P<rtt>\d+)\s+'
        ...
        rtt_sum_average    = 0.0
        cwnd_sum_average   = 0.0
        netpwr_sum_average = 0.0

        for count in range(0,len(interval_sums)-1):
          i = interval_sums[count]
          interval_time = i['interval'].split('-')
          interval_start = float(interval_time[0])
          interval_end = float(interval_time[1])
          # print(thread_results_for_interval)
          interval_start_list.append(interval_start)
          sum_list.append(i['throughput'])

          thread_results_for_interval = list(filter(lambda x: x['interval']==i['interval'], interval_threads))
          ...
          for thread_result in thread_results_for_interval:
            rtt_average += float(thread_result['rtt'])
            cwnd_average += float(thread_result['cwnd'])
            netpwr_average += float(thread_result['netpwr'])
            retry_sum += float(thread_result['retry'])

          rtt_average = rtt_average/len(thread_results_for_interval)
          cwnd_average = cwnd_average/len(thread_results_for_interval)
          netpwr_average = netpwr_average/len(thread_results_for_interval)

          rtt_sum_list.append(rtt_average)
          cwnd_sum_list.append(cwnd_average)
          netpwr_sum_list.append(netpwr_average)
          retry_sum_list.append(retry_sum)

        # END for count in range(0,len(interval_sums)):

        rtt_sum_average = sum(rtt_sum_list)/len(rtt_sum_list)
        cwnd_sum_average = sum(cwnd_sum_list)/len(cwnd_sum_list)
        netpwr_sum_average = sum(netpwr_sum_list)/len(netpwr_sum_list)
        total_sum = interval_sums[len(interval_sums)-1]
        total_throughput = total_sum['throughput']
        ...
        tcp_metadata = {
            ...
            'write_packet_count': total_sum['write'],
            'err_packet_count': total_sum['err'],
            'retry_packet_count': total_sum['retry'],
            'congestion_window': cwnd_sum_average,  ####################
            # 'congestion_window_unit': ,
            'interval_length_seconds': FLAGS.iperf_interval,
            # 'interval_start': interval_start,
            # 'interval_end': interval_end,
            # 'thread_results': thread_results_for_interval,
            'rtt': rtt_sum_average,  #################
            'rtt_unit': thread_results_for_interval[0]['rtt_unit'],
            'netpwr': netpwr_sum_average,  ##############
            'transfer': total_sum['transfer'],
            'interval_throughput_list': sum_list,
            'interval_start_time_list': interval_start_list,
        ...
        return sample.Sample('Throughput', total_throughput, 'Mbits/sec', metadata_tmp)
        ...
        r = re.compile((
            ...
            r'(?P<write>\d+)\/(?P<err>\d+)\s+(?P<retry>\d+)\s+(?P<cwnd>-?\d+)(?P<cwnd_unit>\w*)\/'
            r'(?P<rtt>\d+)\s+(?P<rtt_unit>\w+)\s+(?P<netpwr>\d+\.?\d*)'))
        interval_throughputs = [m.groupdict() for m in r.finditer(stdout)]
        ...
        for count in range(0,len(interval_throughputs)-1):
          i = interval_throughputs[count]
          interval_time = i['interval'].split('-')
          interval_start = float(interval_time[0])
          interval_end = float(interval_time[1])
          interval_start_list.append(interval_start)
          interval_list.append(i['throughput'])
          rtt_sum_list.append(i['rtt'])
          cwnd_sum_list.append(i['cwnd'])
          netpwr_sum_list.append(i['netpwr'])
          retry_sum_list.append(i['retry'])

        total_stats = interval_throughputs[len(interval_throughputs)-1]
        tcp_metadata = {
            'buffer_size': buffer_size,
            'tcp_window_size': window_size,
            ...
            'interval_throughput_list': interval_list,
            'interval_start_time_list': interval_start_list,
            'interval_rtt_list': rtt_sum_list,  #################
            'interval_cwnd_list': cwnd_sum_list, ################
            'interval_retry_list': retry_sum_list, ################
    ...
    else: #FLAGS.iperf_interval == None

      multi_thread = re.search((
          r'\[SUM\]\s+\d+\.\d+-\d+\.\d+\s\w+\s+(?P<transfer>\d+\.?\d*)\s\w+\s+(?P<throughput>\d+\.?\d*)'
