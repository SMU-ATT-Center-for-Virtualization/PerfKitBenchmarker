...
def _RunIperf(sending_vm, receiving_vm, receiving_ip_address, thread_count,
    ...
    if protocol == TCP:
        ...
        window_size = 0
        # Check if search retured NoneType
        if window_size_match:
            window_size = float(window_size_match.group('size'))
        else:
            print(r'Window size match not found')
        ...
        # if single thread
            ...
            if match:
                ...
                print('single thread match not found')
                write = 0
                err = 0
                retry = 0

        print('write: ' + str(write) + ' err: ' + str(err) + ' retry: ' + str(retry))
        r = re.compile((
        ...
        jitter_match = re.search(r'Mbits/sec\s+\d+\.?\d+\s+(?P<jitter_unit>[a-zA-Z]+)', stdout).group('jitter_unit')

        jitter_unit = 0
        if jitter_match:
            jitter_unit = str(jitter_match)
